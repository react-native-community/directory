...
import { type MetadataEntryType, type TemplateType } from '~/types';
...
function generateData({ github, npm, npmPkg }: TemplateType): MetadataEntryType[] {
  return [
    npm?.downloads
      ? {
          id: 'downloads',
...
function generateSecondaryData(library: TemplateType, skipExamples: boolean): MetadataEntryType[] {
...
export default function MetaData({ template, secondary, skipExamples = false }: Props) {
  if (secondary) {
    const data = generateSecondaryData(template, skipExamples).filter(Boolean);
    return (
      <>
        {data
          .filter(entry => !!entry)
    ...
    const data = generateData(template).filter(entry => !!entry);
